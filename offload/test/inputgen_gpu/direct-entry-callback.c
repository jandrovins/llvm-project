...
// RUN:   -I%inputgen-gpu-llvm-include \
// RUN:   %inputgen-gpu-src/inputgen_gpu_entry_state.c \
...
int main(void) {
  ...
  int64_t Generated = __instrumentor_post_load(123, 4, IntegerTyID, 11);
  printf("generate=%lld buffer0=%d\n", (long long)Generated, Buffer[0]);

  Buffer[0] = 42;
  InputGenEntryMode = INPUTGEN_MODE_REPLAY;
  int64_t Replayed = __instrumentor_post_load(123, 4, IntegerTyID, 12);
  printf("replay=%lld\n", (long long)Replayed);

  printf("passthrough-size=%lld\n",
         (long long)__instrumentor_post_load(55, 8, IntegerTyID, 13));
  printf("passthrough-type=%lld\n",
         (long long)__instrumentor_post_load(66, 4, PointerTyID, 14));
  return 0;
}
